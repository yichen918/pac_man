...
static void init(void) {
	...
	basic_map = create_map("Assets/map_2023.txt");
	if (!basic_map) {
...
static void checkItem(void) {
    ...
    case 'P':
        ...
        for(int i=0;i<GHOST_NUM;i++)
        {
            ...
            else ghost_toggle_FLEE(ghosts[i],false);
		}
		if(fleestatus) al_start_timer(power_up_timer);
...
static void status_update(void) {
	for (int i = 0; i < GHOST_NUM; i++) {
        ...
        if(ghosts[i]->status==FLEE && !cheat_mode && RecAreaOverlap(pmanHB,ghostHB))
        {
            game_log("collide with ghost\n");
            ghost_collided(ghosts[i]);
            break;
        }
        if(ghosts[i]->status==FREEDOM && !cheat_mode && RecAreaOverlap(pmanHB,ghostHB))
        {
            game_log("collide with ghost\n");
            al_rest(1.0);
            game_over = true;
...
static void update(void) {
	...
	for (int i = 0; i < GHOST_NUM; i++)
		...
		//if(game_main_Score==2000) ghosts[i]->status=FROZEN;

    //printf("%d %d %d", game_main_Score, basic_map->beansCount, basic_map->powerNum);


    if(al_get_timer_count(power_up_timer))
    {
        int shinetime = al_get_timer_count(power_up_timer);
        if(shinetime==power_up_duration)
        {
            pman->powerUp = false;
        ...
        else if(shinetime >= power_up_duration-2) shine = true;
    }

    if(game_main_Score>highestscore) highestscore = game_main_Score;

    if(game_main_Score==(basic_map->beansCount*10+basic_map->powerNum*50))
    {
...
static void draw(void) {
    ...
    al_draw_text(menuFont,al_map_rgb(200,200,50),SCREEN_H-200,SCREEN_W-100,ALLEGRO_ALIGN_CENTER,str);
